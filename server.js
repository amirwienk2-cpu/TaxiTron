...
function verifyTelegramInitData(initData) {
  ...
  const name = [user.first_name, user.last_name].filter(Boolean).join(' ').trim() || user.username || null;
  return { uid: String(user.id), name };
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth') {
      ...
      const { uid, name } = verifyTelegramInitData(initData);
      const token = issueToken(uid);
      const u = getUser(uid);
      if (name) u.name = name; // keep the leaderboard display name current
      saveDb();
    ...
    /* ---- GET /api/leaderboard?token=... -> { top, you } ----
     * Real, cross-device tournament standings — ranks every player by their
     * best single-run zombie count (`best`, already tracked per user by
     * /api/run). `token` is optional: pass it to also get the requesting
     * player's own rank/best even if they're outside the top 10.
     */
    if (req.method === 'GET' && url.pathname === '/api/leaderboard') {
      const entries = Object.keys(db)
        .filter((k) => k !== '__meta')
        .map((uid) => ({ uid, name: db[uid].name || ('Player' + uid.slice(-4)), best: db[uid].best || 0 }))
        .sort((a, b) => b.best - a.best);

      const top = entries.slice(0, 10).map(({ name, best }) => ({ name, best }));
      ...
      if (token) {
        try {
          ...
          if (idx !== -1) you = { name: entries[idx].name, best: entries[idx].best, rank: idx + 1 };
        } catch (e) { /* no valid token -> just skip "you" */ }
      }

      return sendJson(res, 200, { top, you });
    }
